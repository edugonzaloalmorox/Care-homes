...
cqc_clean =  cqc_clean %>% mutate(wave = ifelse(date <="2015-05-31", "1",
...
geographic = cqc_geo %>% select(location.id, oslaua) %>% unique()

test = left_join(cqc_clean, geographic, by = c("location.id"))



write.csv(test, "/Users/Personas/My Cloud/PhD _october_2016/market entry/care_homes/data/waves/three/cqc_geolocation_waves.csv", row.names = FALSE)
...
exit_all = test %>% filter(registry == "location.end") %>% select(location.id,registry, date, postcode, wave, oslaua)
...
exits_final$wave = as.numeric(exits_final$wave)
...
entry_all = entry_all %>% mutate_each(funs(as.Date), date) %>% arrange(date)

# create the waves

entry_all= entry_all%>% mutate(wave = ifelse(date <="2015-05-31", "1",
                                             ifelse(date >= "2015-06-01" & date <= "2016-02-29" , "2",
                                                    ifelse(date >= "2016-03-01", "3", "4"))))
...
entries_final$wave = as.numeric(entries_final$wave)
...
test_entries = test_entries %>% group_by(oslaua) %>% fill(old.la, la, dclg.code)
...
test_exits$wave =as.numeric(test_exits$wave)
...
write.csv(test_entries, "/Users/Personas/My Cloud/PhD _october_2016/market entry/care_homes/data/waves/three/entries_district_wave3.csv", row.names = FALSE)
write.csv(test_exits, "/Users/Personas/My Cloud/PhD _october_2016/market entry/care_homes/data/waves/three/exits_district_wave3.csv", row.names = FALSE)
write.csv(entries_exits, "/Users/Personas/My Cloud/PhD _october_2016/market entry/care_homes/data/waves/three/entries.exits_district_wave3.csv", row.names = FALSE)
...
write.csv(entries_exits, "/Users/Personas/My Cloud/PhD _october_2016/market entry/care_homes/data/waves/three/entries_exits_rates_ditrict_wave3.csv", row.names = FALSE)
